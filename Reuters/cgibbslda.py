...
codepath = '/gpfs/home/hus152/work/stat540/Final/cgibbslda/cgibbslda'
dirpath = 'cgibbslda'
resfile = 'cgibbslda_res.txt'
mainresfile = 'cgibbslda_mainres.txt'
seedfile1 = 'cgibbslda_seed1.txt'
seedfile2 = 'cgibbslda_seed2.txt'
...
if 1:
	...
	os.system(cmdtxt)
	# read train time
	samplesize = np.loadtxt(dirpath+'/likelihood.dat')[-1,0]
	runtime = np.loadtxt(dirpath+'/likelihood.dat')[-1,6]
	mcse = np.loadtxt(dirpath+'/likelihood.dat')[-1,7]
	# load gtrtheta and gbeta
	gtrtheta = np.exp(np.loadtxt(dirpath+'/final.theta'))
	D = gtrtheta.shape[0]
	gtrtheta = gtrtheta/(np.sum(gtrtheta,1).reshape(D,1))
	np.savetxt(dirpath+'/gtrtheta.txt',gtrtheta)
	gbeta = np.exp(np.loadtxt(dirpath+'/final.beta'))
	gbeta = gbeta/np.sum(gbeta,0)
	...
	os.system(cmdtxt)
	# read test theta
	gttheta = np.exp(np.loadtxt(dirpath+'/testfinal.theta'))
	D = gttheta.shape[0]
	gttheta = gttheta/(np.sum(gttheta,1).reshape(D,1))
	...
	tccr = funcs.classifier_test(tlbls,gtpc_lbl_distn,gttheta)
	# write results
	temp = np.array([runtime, tlkh, 100.0*tccr, mcse, samplesize]).reshape(1,5)
	fp = open(mainresfile,'ab')
	np.savetxt(fp,temp,'%d %5.5f %5.5f %.5e %d')
	fp.close()	
...
for b in range(t0,T):
	...
	# write results
	temp = np.array([runtime, tlkh, 100.0*tccr, beta_mse, theta_mse, mcse, samplesize]).reshape(1,7)
	fp = open(resfile,'ab')
	np.savetxt(fp,temp,'%d %5.5f %5.5f %.5e %.5e %.5e %d')
	fp.close()	
